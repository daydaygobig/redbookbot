...
def main():
    ...
    system = (
        "你是小红书账号「有词儿念念」的选题日报生成器。严格遵循下面的规则文件生成日报。\n"
        "要求：\n"
        "1) 输出纯 Markdown，不要任何解释性开场白，第一行必须是 '# 有词儿念念选题日报 <日期>'\n"
        "2) 严格遵守规则文件的日报输出格式（热点速览/10题/暴论候选），不输出任何数据口径说明\n"
        "3) 参考帖必须从候选清单里选，链接原样复制（24位ID一个字符都不能改）\n"
        "4) 互动数据引用时保留模糊值原样（如 3w+）\n"
        "5) 过滤纯娱乐内容（明星/游戏/二次元/K12课程等），只取职场/成长/搞钱相关信号\n"
        "6) 主航道题不少于6个，模板裂变题不少于3个，与历史清单重复的题不许出现\n\n"
        "===== 规则文件 rules.md =====\n" + rules +
        "\n\n===== 历史已出题目（禁止重复）=====\n" + recent
    )
    user = (
